...
def preprocess_train_data(data_path):
    ...
    with open(data_path, 'r') as f:
        ...
        trainres = []
        idx = 0
        random.shuffle(lines) # finetune use random shuffle
...
class get_dataset(Dataset):
    ...
    def __init__(self, data_lst, tokenizer, mode='train'):
        ...
        #self.csv_file = 'train_df.csv'
        if mode=='train' and os.path.exists('data.pkl'):
            # If the CSV file exists, read it
            logging("Reading df in local address...")
            self.df = pd.read_pickle('data.pkl')
            # Remove the first 62463 rows and keep the rest
            self.df = self.df.iloc[:20000]  # start from index 62463 to the end

            # Optionally, you can reset the index if you want it to start from 0
            self.df.reset_index(drop=True, inplace=True)
            #self.df = pd.read_csv(self.csv_file)
            #print(self.df.iloc[1]['output_ids'])
            ...
            if len(self.df):
                ...
                if mode == 'train':
                    logging("Saving df in local address")
...
def load_datasets(args, tokenizer, mode='train'):
    ...
    if mode == 'train':
        ...
        _, train_lst, _ = preprocess_test_data(args.dev_data_path, mode='train')
...
def load_dataloaders(args, mode='train'):
    ...
    if mode=='train':
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, \
                                    collate_fn=PS, pin_memory=False)#, sampler=DistributedSampler(train_set))
        dev_loader = DataLoader(dev_set, batch_size=args.batch_size//2, shuffle=False, \
                                collate_fn=PS, pin_memory=False)
        test_loader = 0
