...
class Config(object):
    ...
    loss_alpha_rej_alpha = 0 # in mlm_cls, loss = \alpha * {the words that keet unchanged} + {the words that is substituted}
    ...
    exp_id = 'dpo_margin_sow'

    # training arguments -------------------------------------------------------

    gpuid = '0'

    batch_size = 32 if 'mlm' not in model_choice else 50
    test_batch_size = 1
    ...
    train_use = 3900000 # use [:train_use] data for train
    save_model = 1
    print_log = 1
    ...
    data_path = '../../../data/sws_ds/wiki_170_s-lexical-1-1125.json'
    dev_data_path = '../SmartWordSuggestions/data/sws/sws_eval.json'
    test_data_path = '../SmartWordSuggestions/data/sws/sws_test.json'
    ...
    output_path += '/' + nowtime
    if not os.path.exists(output_path): os.mkdir(output_path)
    log_path = os.path.join(output_path, 'finetune_ckpt.log')
