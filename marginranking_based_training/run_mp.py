...
def train(args):
    ...
    Model = load_model()
    #net = torch.nn.DataParallel(Model(args))
    net = Model(args,device)
    load_state(net, None, None, args, load_best=False)  # load trained models for finetune
    net.to(device)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if 'mlm' in args.model_choice:
                loss,total_sum,cosine_correlation, classification_logits = net(input_ids, labels, mask_pos)
            elif args.model_choice == 'seq2seq':
                loss, output_sequence = net(input_ids, labels)

            sum_of_bartscore_ratio.append(total_sum)
            cos_sim.append(cosine_correlation)
        ...
        if gpu_id == 0:
            ...
            try:
                results, (out_labels, true_labels) = evaluate(net, test_loader, args, mode='test')
        ...
        if gpu_id == 0 and (epoch % args.save_epoch) == 0:
            torch.save({
                    'epoch': epoch + 1,\
                    'state_dict': net.module.state_dict(),\
                    #'best_f_e2e_05': f_e2e_05_per_epoch[-1],\
...
def infer(args):
    ...
    Model = load_model()
    net = Model(args, 'cuda:0')
    if torch.cuda.is_available(): net.to('cuda:0')
